...
static double
rand_uniform( double lo, double hi )
{
...
static void
apply_wind( struct hermes_env *env )
{
...
static void
apply_action( struct hermes_env *env, enum hermes_action act )
{
        if ( act == HERMES_ACTION_RIGHT )
                env->speed += HERMES_ACTION_ACCELERATION * HERMES_DT;
        else if ( act == HERMES_ACTION_LEFT )
                env->speed -= HERMES_ACTION_ACCELERATION * HERMES_DT;
}

static void
apply_friction( struct hermes_env *env )
{
        if ( env->speed > 0.0 ) {
...
static void
advance_position( struct hermes_env *env )
{
        env->position += env->speed * HERMES_DT;
}

static void
tick_wind_timer( struct hermes_env *env )
{
        if ( env->wind_dir == 0 ) return;
...
static void
maybe_restart_wind( struct hermes_env *env )
{
        if ( env->wind_dir != 0 ) return;
        ...
        env->wind_remaining =
            rand_uniform( HERMES_WIND_MIN_DURATION, HERMES_WIND_MAX_DURATION );
}

// === --- Public API --------------------------------------------------- ===

void
hermes_env_init( struct hermes_env *env )
{
        env->position       = 0.0;
        env->speed          = 0.0;
        env->wind_dir       = 0;
...
void
hermes_env_deinit( struct hermes_env *env )
{
        (void)env;
}

void
hermes_env_obs( const struct hermes_env *env, double *pos_out,
                double *speed_out )
{
...
int
hermes_env_action( struct hermes_env *env, enum hermes_action act,
                   double *reward_out )
{
