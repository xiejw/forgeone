...
static void
print_frame( int step, double pos, double speed, enum hermes_action act,
             int wind_dir )
{
        char track[3 + HERMES_TRACK_CELLS];
        render( pos, track );
        printf( "step %3d %s pos=%+6.1f speed=%+5.1f act=%-5s " CYAN "%s" RESET
                "\n",
...
double
hermes_run_episode( struct hermes_env *env, hermes_policy_fn policy,
                    int verbose )
{
