...
static enum hermes_action counting_random(double pos, double speed)
{
        step_count++;
        return hermes_random_policy(pos, speed);
}

static enum hermes_action counting_rev(double pos, double speed)
{
        step_count++;
        return hermes_rev_policy(pos, speed);
}

static int check_policy(const char *name, hermes_policy_fn fn)
{
        step_count = 0;
        double reward = hermes_run_episode(fn, 0);

        if (!(reward >= 1.0 && reward <= (double)HERMES_MAX_STEPS)) {
                fprintf(stderr, "[%s] reward %.1f out of [1, %d]\n",
                        name, reward, HERMES_MAX_STEPS);
                return 1;
        }
        if (step_count < 1 || step_count > HERMES_MAX_STEPS) {
                fprintf(stderr, "[%s] step_count %d out of [1, %d]\n",
                        name, step_count, HERMES_MAX_STEPS);
                return 1;
        }
