...
/// fixed-size arrays (no heap). Weight matrices are row-major:
/// `w1[h * NN_IN + i]` is the weight from input `i` to hidden unit `h`.
pub struct HermesNn {
    // Parameters.
    w1: [f32; NN_HID * NN_IN],
    b1: [f32; NN_HID],
...
impl HermesNn {
    ...
    pub fn new(seed: u32) -> HermesNn {
        ...
        for i in 0..nn.w1.len() {
            nn.w1[i] = nn.rng.uniform_sym() * NN_INIT_SCALE;
        }
        for i in 0..nn.w2.len() {
            nn.w2[i] = nn.rng.uniform_sym() * NN_INIT_SCALE;
        }
        nn.compile();
        nn
    }
    ...
    pub fn forward_probs(&mut self, pos: f32, speed: f32) -> [f32; NN_OUT] {
        let prog = self.forward.clone();
        let mut vm = Vm::new(self, [pos, speed]);
        vm.run(&prog);
        vm.probs
    }
    ...
    pub fn zero_grad(&mut self) {
        self.g_w1 = [0.0; NN_HID * NN_IN];
        self.g_b1 = [0.0; NN_HID];
        self.g_w2 = [0.0; NN_OUT * NN_HID];
        self.g_b2 = [0.0; NN_OUT];
    }

    /// Applies a single SGD update `p -= lr * g` across all parameters.
    pub fn sgd_step(&mut self, lr: f32) {
        for (w, g) in self.w1.iter_mut().zip(self.g_w1.iter()) {
            *w -= lr * g;
        }
        for (w, g) in self.b1.iter_mut().zip(self.g_b1.iter()) {
            *w -= lr * g;
        }
        for (w, g) in self.w2.iter_mut().zip(self.g_w2.iter()) {
            *w -= lr * g;
        }
        for (w, g) in self.b2.iter_mut().zip(self.g_b2.iter()) {
            *w -= lr * g;
        }
    }
...
/// Execution state for one program run. Each value owns its data in `tensors`;
/// the stack and tape hold `TensorId`s into that registry.
struct Vm<'a> {
    nn: &'a mut HermesNn,
...
impl<'a> Vm<'a> {
    fn new(nn: &'a mut HermesNn, input: [f32; NN_IN]) -> Vm<'a> {
        Vm {
            nn,
            tensors: Vec::new(),
            stack: Vec::new(),
    ...
    fn op_linear(&mut self, instr: Instr) {
        let x_id = self.pop();
        self.tape_push(x_id);
        let x = self.val(x_id).to_vec();

        let (w, rows, cols) = self.nn.weight(instr.a);
        let b = self.nn.bias(instr.b);
    ...
    fn op_tanh(&mut self) {
        let z_id = self.pop();
        let z = self.val(z_id).to_vec();
        let out: Vec<f32> = z.iter().map(|v| v.tanh()).collect();
        let id = self.alloc(out);
        self.push(id);
        self.tape_push(id);
    }
    ...
    fn op_dlinear(&mut self, instr: Instr) {
        let g_id = self.pop();
        let g = self.val(g_id).to_vec();
        let x_id = self.tape_pop();
        let x = self.val(x_id).to_vec();

        let gx = {
            let (w, gw, gb, rows, cols) = self.nn.dlinear_refs(instr.a, instr.b);
